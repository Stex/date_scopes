...
module SimpleDateScopes
  ...
  def self.included(base_class)
    ...
    base_class.class_eval do
      ...
      scope :yesterday, ->(f = :created_at) { in_x_of(Date.today - 1.day, f, :day) }
      scope :today,     ->(f = :created_at) { in_x_of(Date.today, f, :day) }
      scope :tomorrow,  ->(f = :created_at) { in_x_of(Date.today + 1.day, f, :day) }
